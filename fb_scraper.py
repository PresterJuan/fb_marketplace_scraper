...
def search(browser, term_to_search):
    search_input = browser.find_element_by_xpath("//input[@placeholder='Search Marketplace']")
    ...
    for i in imgs:
        try:
            ...
            if 'San Antonio' in alt_text or 'Austin' in alt_text or 'Houston' in alt_text:
                ...
                stuff['url'] = 'https://www.facebook.com' + i.find_previous('a')['href']
...
def save_to_db(dict_instance, table_name, name_of_db):
    ...
    ban_list_check = c.execute(f"SELECT * FROM ban_list WHERE url=(?)", (url_to_search,))
    banned = ban_list_check.fetchone()
    if banned != None:
        print('\n\n\nFound band url:', dict_instance)
        return
    null_test = c.execute(f"SELECT * FROM listings WHERE url=(?)", (url_to_search,))
    old_row = null_test.fetchone()
    if old_row == None:
        new_tuple = (int(dict_instance['cost'][1:].replace(',' , '')),dict_instance['description'],dict_instance['url'], datetime.date.today())
        c.executemany(f'INSERT INTO {table_name} VALUES (?,?,?,?)', {new_tuple})
    else:
        if old_row[0] == int(dict_instance['cost'][1:].replace(',' , '')):
            ...
            price_change = int(dict_instance['cost'][1:].replace(',' , '')) - old_row[0]
            change_tuple = (dict_instance['url'],dict_instance['description'],price_change, datetime.date.today())
            c.execute(f'INSERT INTO price_change VALUES (?,?,?,?)', {change_tuple})
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print('\n\n\npayload grabbed')
        print(payload)
        new_payload = screen_out(payload,('mini','ipad','imac','dell','fridge','cooler','bar','keyboard'))
        print('\n\n\nnew_payload worked')
        print(new_payload)
